...
def generate_csv(csv_file, csv_path, audio_path, category):
    ...
    df.to_csv(csv_path + "CREMA_with_audio_path.csv")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--csv_file", default="CREMA_no_path.csv", type=str, required=True)  # Path of csv to load
    parser.add_argument("--csv_path", default=None, type=str, required=True)  # Path to save the csv file
    parser.add_argument('--audio_path',default=None,type =str, required=True) # csv file name to save
    ...
    generate_csv(csv_file, csv_path, audio_path, category) #will generate csv file with audio name and audio path
